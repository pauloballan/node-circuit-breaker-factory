...
exports.create = function(config, logger) {
  ...
  valid_params = internals.validateCreateParams({
    config: config,
    logger: logger
  });
...
internals.createCircuitBreakerOptsFromConfig = function(config, logger) {
  var curried_log_function, valid_config,
  cache = {},
  opts;

  valid_config = internals.validateConfigParams(config);
...
internals.validateCreateParams = function(params) {
  return validator.validateSchema(params,
