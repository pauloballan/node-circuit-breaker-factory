...
exports.create = function(config, logger) {
  var valid_params = internals.validateCreateParams({
    config: config,
    logger: logger,
  });

  var breaker_opts = internals.createCircuitBreakerOptsFromConfig(valid_params.config, valid_params.logger);
  var breaker = new CircuitBreaker(breaker_opts);
...
internals.createCircuitBreakerOptsFromConfig = function(config, logger) {
  var valid_config = internals.validateConfigParams(config);
  var opts = {
  ...
  var cache = {};
  var curried_log_function = internals.logStateChange.bind(internals.logStateChange, logger, cache, valid_config.source_name, valid_config.target_name);
